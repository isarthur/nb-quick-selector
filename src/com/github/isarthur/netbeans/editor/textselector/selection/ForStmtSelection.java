 ...
 */
public class ForStmtSelection extends Selection {

    public ForStmtSelection(JTextComponent editor, TokenSequence<?> tokenSequence, int selectionStart,
            int selectionEnd,
    ...
    public void select() {
        ...
        TreePath forStatementPath = treeUtilities.pathFor(tokenSequence.offset() + 1);
        if (forStatementPath == null) {
            return;
        }
        Tree forStatementTree = forStatementPath.getLeaf();
        Trees trees = controller.getTrees();
        SourcePositions sourcePositions = trees.getSourcePositions();
        CompilationUnitTree compilationUnitTree = controller.getCompilationUnit();
        long startPosition = sourcePositions.getStartPosition(compilationUnitTree, forStatementTree);
        long endPosition = sourcePositions.getEndPosition(compilationUnitTree, forStatementTree);
